...
export const BucketItemCard = ({
  ...
  return (
    <article className="glass-panel group overflow-hidden p-5 transition duration-300 hover:-translate-y-1 hover:shadow-[0_28px_90px_rgba(20,33,61,0.14)]">
      <div className="flex flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {item.collaboratorIds.length > 0 ? (
                <span className="inline-flex items-center gap-1 rounded-full bg-cloud-100 px-3 py-1 text-xs font-semibold text-ink-800">
                  ...
                  {item.collaboratorIds.length} collaborator
                  {item.collaboratorIds.length > 1 ? 's' : ''}
                </span>
              ) : null}
            </div>
            <h3 className="font-display text-3xl text-ink-950">{item.title}</h3>
          ...
          {canEdit ? (
            <div className="flex flex-wrap justify-end gap-2">
              <button type="button" onClick={() => onEdit(item)} className="action-chip">
              ...
              <button
                ...
                className="action-chip text-rose-400"
              >
        ...
        <div className="flex flex-wrap gap-4 text-sm text-ink-800/72">
          <span>Created {formatDate(item.createdAt)}</span>
          <span>Owner: {item.userId}</span>
        </div>
